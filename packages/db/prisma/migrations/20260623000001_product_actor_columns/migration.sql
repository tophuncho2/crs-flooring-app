-- ============================================================================
-- FlooringProduct: add `createdBy` / `updatedBy` actor-email columns.
--
...
-- user (mirroring the FlooringJobType / FlooringPayment / FlooringWarehouse actor
-- precedent — a plain string, no FK).
--
...
--
-- NOTE: FlooringProduct createdBy/updatedBy have NO @map, so their real names
-- are camelCase and MUST be double-quoted — an unquoted identifier folds to
-- lowercase.
-- ============================================================================

ALTER TABLE "flooring_product" ADD COLUMN "createdBy" TEXT;
ALTER TABLE "flooring_product" ADD COLUMN "updatedBy" TEXT;
