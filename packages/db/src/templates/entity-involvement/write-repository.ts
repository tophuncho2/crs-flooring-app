...
import type { TemplateEntityInvolvementForm } from "@builders/domain"
...
export type WriteTemplateEntityInvolvementInput = TemplateEntityInvolvementForm

export type ApplyTemplateEntityInvolvementsDiffInput = {
  templateId: string
  ...
  actorEmail: string
  added: Array<{ id: string; tempId: string; input: WriteTemplateEntityInvolvementInput }>
  modified: Array<{ id: string; input: WriteTemplateEntityInvolvementInput }>
  deleted: Array<{ id: string }>
}

export type ApplyTemplateEntityInvolvementsDiffResult = {
  tempIdMap: Record<string, string>
}

export async function applyTemplateEntityInvolvementsDiff(
  tx: Prisma.TransactionClient,
  input: ApplyTemplateEntityInvolvementsDiffInput,
): Promise<ApplyTemplateEntityInvolvementsDiffResult> {
  if (input.deleted.length > 0) {
    await tx.templateEntityInvolvement.deleteMany({
  ...
  if (input.added.length > 0) {
    await tx.templateEntityInvolvement.createMany({
  ...
  for (const update of input.modified) {
    ...
    const data: Prisma.TemplateEntityInvolvementUncheckedUpdateInput = {
      entityId: update.input.entityId,
      involvementType: update.input.involvementType ? update.input.involvementType : null,
      updatedBy: input.actorEmail,
    }
    await tx.templateEntityInvolvement.update({ where: { id: update.id }, data })
  }
