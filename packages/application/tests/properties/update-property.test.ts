...
import { updatePropertyUseCase } from "../../src/properties/update-property.js"

const ID = "prop-1"
const ACTOR = "actor@example.com"

function detail(overrides: Record<string, unknown> = {}) {
  return { id: ID, name: "Maple Court", createdBy: null, updatedBy: null, ...overrides }
}
...
describe("updatePropertyUseCase", () => {
  it("rejects a blank actorEmail before touching the database", async () => {
    await expect(updatePropertyUseCase(ID, { phone: "x" } as never, "   ")).rejects.toThrowError(
      /actorEmail/,
    )
    expect(updatePropertyRecordMock).not.toHaveBeenCalled()
  })

  it("updates without a name when name is omitted, stamping updatedBy", async () => {
    await updatePropertyUseCase(ID, { phone: "555-1212" } as never, ACTOR)
    expect(updatePropertyRecordMock).toHaveBeenCalledWith(
      ID,
      { phone: "555-1212", updatedBy: ACTOR },
      expect.anything(),
    )
  })

  it("rejects a blank name when one is supplied", async () => {
    await expect(updatePropertyUseCase(ID, { name: "   " } as never, ACTOR)).rejects.toMatchObject({
  ...
  it("sends the name through when it changes", async () => {
    await updatePropertyUseCase(ID, { name: "New Name" } as never, ACTOR)
    expect(updatePropertyRecordMock).toHaveBeenCalledWith(
      ID,
      expect.objectContaining({ name: "New Name", updatedBy: ACTOR }),
      expect.anything(),
    )
  })
  ...
  it("maps a P2025 to a 404 not-found", async () => {
    ...
    await expect(
      updatePropertyUseCase(ID, { name: "New Name" } as never, ACTOR),
  ...
  it("returns the updated record on success", async () => {
    ...
    expect(await updatePropertyUseCase(ID, { phone: "x" } as never, ACTOR)).toBe(updated)
  })
})
