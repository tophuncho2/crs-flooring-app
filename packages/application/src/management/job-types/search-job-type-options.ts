import { searchJobTypeOptions } from "@builders/db"
import type { JobTypeOption } from "@builders/domain"

export type SearchJobTypeOptionsInput = {
  search?: string
  take?: number
}

const OPTIONS_DEFAULT_TAKE = 20
const OPTIONS_MAX_TAKE = 50

export async function searchJobTypeOptionsUseCase(
  input: SearchJobTypeOptionsInput,
): Promise<JobTypeOption[]> {
  const search = input.search?.trim() || undefined
  const requested = Math.floor(input.take ?? OPTIONS_DEFAULT_TAKE)
  const take = Math.max(1, Math.min(OPTIONS_MAX_TAKE, requested))
  return searchJobTypeOptions({ search, take })
}
