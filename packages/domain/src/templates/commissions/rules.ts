...
// row may carry any combination of an entity link + percent + notes. The only rule:
// a provided percent must be a valid scale-3 percent ≤ 100 (defense — the API
// validator normalizes too). Returns "" when valid, else a human message (the
// templates child-section convention). Entity referential validity is the FK's job.
