...
- Location: `packages/db/scripts/backfill-product-stock-unit.ts` (run via `tsx`, same as prior one-off scripts).
...
- Print: total, matched-by-name, matched-by-category, unresolved IDs. Idempotent (only sets where null).
...
**Resolve unit at read (templates / WO items):**
- `packages/db/src/management/templates/material-items/read-repository.ts` and `packages/db/src/flooring/work-orders/material-items/read-repository.ts` + `work-orders/read-repository.ts` (file-gen path) — drop the item's own `sendUnit*` select; join `product.stockUnit` and surface `stockUnitName`/`stockUnitAbbrev` (resolved).
- Domain item types (`management/templates/material-items/types.ts`, `flooring/work-orders/material-items/types.ts`) — replace `sendUnitName/Abbrev` with `stockUnitName/Abbrev`.
...
No UoM picker exists today (unit was static). Mirror the manufacturer picker head-to-toe:
- `packages/db/src/flooring/unit-of-measures/read-repository.ts` — add `searchUnitOfMeasureOptions(search, skip, take)` (`take+1` paging) alongside existing `listUnitOfMeasures`.
- `packages/application/src/flooring/unit-of-measures/search-unit-of-measure-options.ts` — new use case + validator.
- `apps/web/app/api/unit-of-measures/options/route.ts` — GET, full gauntlet, returns `{ items, hasMore }`.
...
Categories are seed-only today (no write repo, no mutations). Mirror the product CRUD + gauntlet:
- `packages/domain/src/flooring/categories/normalizers.ts` — add `slugifyCategoryName(name)` (lowercase, trim, collapse whitespace → hyphens). The single source of truth for slug derivation.
- `packages/db/src/flooring/categories/write-repository.ts` — `createCategory` / `updateCategory` / `deleteCategory`. Create + update both set `slug = slugifyCategoryName(name)` (regenerate on rename); map the unique-violation to a friendly duplicate error.
- `packages/application/src/flooring/categories/{create,update,delete}-category.ts` — use cases (guard delete when products reference the category).
