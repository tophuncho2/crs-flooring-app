...
This is an **editing** skill — it reads, decides the skill's shape, then writes the file. It is not a read-only recon (that's `/report`/`/dig`) and not a build gauntlet (that's `/check`).
