...
Think of it as the inverse of `/newsession`: that skill reads entire modules end-to-end before producing a full layered plan. `/quick-report` reads the minimum to be right and gives a terse verdict — which may stand alone, or rip into `/newsession` if it turns out a real change is needed.
...
- **Ultra-short.** Lead with a one-line verdict (✅ / ❌ / ⚠️), then **≤ ~6 checklist bullets**, each with a real `path:line`. No prose paragraphs. No layer headers. No restating the question back at length. If it doesn't fit on a screen, you're doing `/newsession`, not `/quick-report`.
