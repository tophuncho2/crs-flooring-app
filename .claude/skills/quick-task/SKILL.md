...
A `/quick` pass targets **one side**. Spanning both is the exception, not the default.
...
- **One side per pass.** Auto-detect the side from your invoke context + the session diff; state which side and why before editing. You can force it (`/quick backend …`). If the cleanup genuinely spans both, say so and do the side clearly in scope; **flag** the other rather than silently crossing over.
...
- **Never touch schema.** No edits under `packages/db/prisma/`, no migrations. If a cleanup needs a schema change, stop and flag it — out of `/quick` scope.
...
- **Explicit-only.** Trigger on the literal `/quick`. Never on "quick fix", "quickly clean this up", "a quick edit".

## Step 1 — Read the context

Combine two inputs:

1. **Your invoke context** — the prompt after `/quick` (what to clean, and possibly which side).
...
- Commit.
- Trigger on anything but the literal `/quick` invocation.
