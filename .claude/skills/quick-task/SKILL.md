...
This is an **editing** skill. It is not a module sweep (that's `/session-new`), not an audit (that's `/dig`), not the build gauntlet (that's `/check-gauntlet`), and not a cross-layer refactor (that's `/engine` or a real session).
...
- **Verify the touched side only** — a scoped typecheck or the relevant test, not the full `/check-gauntlet` gauntlet unless the user asks.
...
- Run the full `/check-gauntlet` gauntlet unless asked.
- Commit.
