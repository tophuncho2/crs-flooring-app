...
**Unit (vitest with mocks)** — covers the dispatcher state machine and the worker's error classification. Specific test cases:

- `topic-dispatcher.test.ts`: claim → enqueue → mark-dispatched happy path; enqueue throws → retry with backoff until max attempts → exhaust; duplicate enqueue (`wasDuplicate: true`) → still mark dispatched; payload parse failure → exhaust immediately (poison message, no retry); claim returns null (raced) → continue cleanly.
- `materialize-import-batch.test.ts`: happy path with mocked use case → returns result; `StagedInventoryExecutionError` thrown → wraps in `UnrecoverableError`; transport error thrown → re-throws plain (BullMQ retries).

**Staging smoke** — manual end-to-end against the actual Railway staging env. After deploy:

1. Deploy relay + worker to staging via Railway CLI (`railway up --service relay`, `railway up --service "bull mq"` or whatever the Railway service name is).
2. Verify both services boot cleanly (Railway logs show "ready" + no crash loop).
3. From a one-off tsx script (or the eventual sweep-5 route), call `markStagedRowsForImportUseCase` with a small fake batch (2–3 rows, real productIds + locationIds in staging). Confirm:
   - Outbox row appears in `queue_outbox_event` with status `PENDING`, then `DISPATCHED` within ~2s.
   - Bull Board (relay's `/admin/queues` or whatever the path is) shows the job arriving in `flooring-imports-materialize` queue.
   - Worker picks it up; structured log `event: "job.completed"` appears in worker logs.
   - Inventory rows exist with the expected snapshot fields (`coveragePerUnit`, `costPerUnit`, unit names).
   - Staged rows flipped `QUEUED → IMPORTED`.
4. Re-run with the same input. Confirm `wasDuplicate: true` flows through; no duplicate inventory rows; outbox idempotency works.
5. Manually delete one staged row between mark-for-import and worker pickup (simulate drift). Confirm worker dead-letters via `STAGED_MATERIALIZE_PRECONDITION_FAILED`; outbox stays `DISPATCHED`; BullMQ failed set has the job.

**No integration tests this sweep.** Setting up testcontainers (Redis + Postgres) is its own infrastructure decision and isn't materialize-driven. Staging is real Railway with real services — it's the better integration environment for this sweep. We can revisit testcontainers as a separate "test infra" sweep later.

## Critical files to read before executing

- `packages/db/src/queues/outbox-repository.ts` — the data-layer surface the dispatcher orchestrates.
- `packages/application/src/flooring/imports/staged-inventory-rows/materialize-imported-rows.ts` — the use case the worker calls. Confirms input/output shape.
- `packages/domain/src/flooring/imports/staged-inventory-rows/import-batch-payload.ts` — current home of the contract; this file gets moved to `queue/`.
- `apps/relay/src/dispatch/bullmq-idempotent-dispatch.ts` — surviving helper.
- `apps/relay/src/bootstrap.ts` — current single-topic shape; rewriting the dispatch loop.
- `apps/worker/src/bootstrap.ts` — current Worker construction; rewriting for the new queue.
- `package.json` (root) — `build:relay`, `build:worker`, `build:web` scripts confirm per-service build chains exist.
- `apps/relay/src/bull-board.ts:58–60` — confirms `/healthz` endpoint for relay.
...
4. **Staging smoke**: per the procedure above. Bull Board shows the queue, jobs run, inventory materializes.
...
1. **Pre-execution**: I observe the staging Railway env (read-only — `railway logs`, `railway status`) to confirm current crash signatures and that the recon's analysis matches reality.
2. **Code changes**: domain → relay → worker, in that order (each builds on the prior).
3. **Railway configs**: land last, since they're the deploy unblock. Test in staging.
4. **Sweep 5 unblocks afterwards**: apps/web rewires can now ship without being gated on the relay/worker work.
