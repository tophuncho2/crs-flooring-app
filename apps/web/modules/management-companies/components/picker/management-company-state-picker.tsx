...
import {
  MANAGEMENT_COMPANY_STATES_SEARCH_QUERY_KEY,
  searchManagementCompanyStatesRequest,
} from "@/modules/management-companies/data/management-company-state-options-request"

export type ManagementCompanyStatePickerProps = {
  value: string | null
...
function toDropdownOption(option: ManagementCompanyStateOption): AsyncRichDropdownOption {
  return {
    id: option.value,
    title: option.value,
  }
...
export function ManagementCompanyStatePicker({
  value,
  onChange,
  selectedLabel = null,
  ...
  className,
  initialOptions,
}: ManagementCompanyStatePickerProps) {
  const bucketKey = useMemo(
    () => [...MANAGEMENT_COMPANY_STATES_SEARCH_QUERY_KEY] as const,
    [],
  )

  const searchFn = useCallback(
    (search: string, signal: AbortSignal | undefined) =>
      searchManagementCompanyStatesRequest(search, signal),
    [],
  )

  const controller = useAsyncRichDropdownController<ManagementCompanyStateOption>({
    bucketKey,
    searchFn,
    initialOptions,
    enabled: !disabled,
