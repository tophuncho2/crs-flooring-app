...
export type AddPropertyButtonProps = {
  onClick: () => void
  disabled?: boolean
}

/**
 * Toolbar action: opens the property side panel in create mode.
 * Card-style button sized to one toolbar row; designed to live stacked
 * above `AddHubButton` inside a single `ListToolbarCell`.
 */
export function AddPropertyButton({ onClick, disabled }: AddPropertyButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
