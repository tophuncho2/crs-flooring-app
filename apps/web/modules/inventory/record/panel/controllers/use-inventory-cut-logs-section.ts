...
export function useInventoryCutLogsSection({
  ...
  const section = useRecordScopedSectionController<InventoryRow, InventoryCutLogDraft | null>({
    ...
    onSave: async (draft, currentRecord) => {
      ...
      await requestJson<{ cutLog: InventoryRow["cutLogs"][number] }>("/api/cut-logs", {
        method: "POST",
      ...
      const payload = await requestJson<{ inventory: InventoryRow }>(`/api/inventory/${currentRecord.id}`)
