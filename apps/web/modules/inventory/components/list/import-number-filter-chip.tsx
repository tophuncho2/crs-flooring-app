...
/**
 * Placeholder chip for an Import # picker. Visual matches the closed-state
 * trigger of `AsyncRichDropdown` so the toolbar reads as a full grid of
 * controls even before the query layer is wired up. Replace with a real
 * picker wrapper (e.g. `<ImportPicker .../>`) once the search endpoint
 * exists.
 */
export function ImportNumberFilterChip() {
  return (
    <button
      type="button"
      disabled
      aria-label="Filter inventory by import number (coming soon)"
