import type {
  CategoriesListFilters,
  ListInput,
  ListOutput,
} from "@builders/application"
import {
  LIST_CATEGORIES_PAGE_SIZE,
  type CategoryListRow,
} from "@builders/domain"
import { requestJson } from "@/transport/http"

function readSearchParam(
  searchParams: Record<string, string | string[] | undefined> | undefined,
  key: string,
): string | undefined {
  const raw = searchParams?.[key]
  return Array.isArray(raw) ? raw[0] : raw
}

// Read-only list — pagination only. `filters: {}` (not `undefined`) so the SSR
// prefetch key matches the engine's constructed listInput and hydration hits.
export function parseCategoriesListInputFromSearchParams(
  searchParams: Record<string, string | string[] | undefined> | undefined,
): ListInput<CategoriesListFilters> {
  const pageRaw = Number(readSearchParam(searchParams, "page"))
  const page = Number.isFinite(pageRaw) && pageRaw >= 1 ? Math.floor(pageRaw) : 1

  return {
    filters: {},
    page,
    pageSize: LIST_CATEGORIES_PAGE_SIZE,
  }
}

export function buildCategoriesListSearchString(
  input: ListInput<CategoriesListFilters>,
): string {
  const params = new URLSearchParams()
  if (input.page && input.page !== 1) params.set("page", String(input.page))
  if (input.pageSize) params.set("pageSize", String(input.pageSize))
  return params.toString()
}

export async function listCategoriesRequest(
  input: ListInput<CategoriesListFilters>,
): Promise<ListOutput<CategoryListRow>> {
  const queryString = buildCategoriesListSearchString(input)
  const url = queryString ? `/api/categories?${queryString}` : "/api/categories"
  return requestJson<ListOutput<CategoryListRow>>(url, {
    method: "GET",
    headers: { Accept: "application/json" },
  })
}

export const CATEGORIES_LIST_QUERY_KEY = ["categories", "list"] as const
