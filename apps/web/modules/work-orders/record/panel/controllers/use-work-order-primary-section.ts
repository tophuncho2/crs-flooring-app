...
import { useRecordScopedSectionController } from "@/modules/shared/engines/record-view"
import {
  areWorkOrderDraftsEqual,
  cloneDraftWorkOrder,
  toWorkOrderDraft,
} from "../shared"
import type { DraftWorkOrder, WorkOrderDetail } from "@/modules/work-orders/types"
...
export function useWorkOrderPrimarySection(input: {
  ...
  return useRecordScopedSectionController<WorkOrderDetail, DraftWorkOrder>({
    ...
    onSave: async (nextDraft, serverWorkOrder, serverRevisionKey) => {
      try {
        const payload = await requestJson<{ workOrder: WorkOrderDetail }>(`/api/work-orders/${serverWorkOrder.id}`, {
          method: "PATCH",
