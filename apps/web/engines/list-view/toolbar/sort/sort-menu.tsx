...
const TRIGGER_CLASS_NAME =
  "inline-flex items-center gap-2 rounded-lg border border-[var(--panel-border)] px-3 py-2 text-sm font-semibold transition hover:bg-[var(--panel-hover)]"
...
/**
 * Multi-level sort builder. Renders a "Sort" toolbar button with an active-level
 * badge that opens a panel where each level picks a column + direction and can be
 * removed; an "Add level" affordance appears while under {@link maxLevels} and
 * unused columns remain. Pure UI — `value`/`onChange` wire straight to the list
 * controller's `sorts` + `onSortsChange`. Column headers stay the quick
 * single-sort path; this composes the ordered chain.
 */
export function SortMenu({
  ...
  return (
    <div ref={rootRef} className={joinClasses("relative z-30", className)}>
      ...
      {isOpen ? (
        <div className="absolute right-0 top-full z-[70] mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-xl border border-[var(--panel-border)] bg-[var(--panel-background)] p-3 shadow-xl">
