...
export type DateCellProps = CellProps<string> & {
  placeholder?: string
}

/**
 * Date cell. Editable: native HTML5 `<input type="date">` (calendar picker;
 * value is `YYYY-MM-DD`). Read-only: formatted via `formatStableDate`.
...
export function DateCell(props: DateCellProps) {
  ...
  return (
    ...
    >
      {props.value ? formatStableDate(props.value) : "-"}
    </span>
  )
}
