...
1. **Every primitive lives under exactly one bucket.** Buckets: `grid/`, `layout-grid/`, `fields/`, `cells/`, `dialogs/`, `nav/`, `badges/`, `headers/`, `features/`. Composites import down the dependency tree, never sideways: `cells/` may import `badges/` and `SelectDropdown` from the picker engine (`@/engines/picker`); `fields/` may import `layout-grid/` and `cells/`; `dialogs/` imports nothing from other buckets; `nav/` imports nothing from other buckets; `grid/` never imports any feature. (Dropdowns are no longer a bucket here — they moved to `@/engines/picker`.)
...
- **dropdowns** — moved out to the picker engine (`@/engines/picker`): `SelectDropdown`, `AsyncRichDropdown`, `SegmentedDropdown`, `AnchoredPanel` + `useAsyncRichDropdownController`. `cells/dropdown-cell.tsx` composes `SelectDropdown` from there.
...
This tree is **scaffolding**. Both view engines now live under `apps/web/engines/` — record-view (`@/engines/record-view`) and list-view (`@/engines/list-view`, which absorbed the old `controllers/list-view/`, `components/data-table/`, and the list `features/` layers). Dropdowns and the picker chrome have likewise moved to the picker engine (`@/engines/picker`). What remains here is pure shared primitives (`grid/`, `layout-grid/`, `fields/`, `cells/`, `dialogs/`, `nav/`, `badges/`, `headers/`, `theme/`) plus the `features/` row side-cars; the engines depend inward on these, never the reverse — the one exception being `cells/dropdown-cell` composing `SelectDropdown` from the picker engine.
