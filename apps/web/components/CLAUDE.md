...
1. **Every primitive lives under exactly one bucket.** Buckets: `grid/`, `layout-grid/`, `fields/`, `cells/`, `dropdowns/`, `badges/`, `headers/`, `features/`. Composites import down the dependency tree, never sideways: `cells/` may import `dropdowns/` and `badges/`; `fields/` may import `layout-grid/` and `cells/`; `dropdowns/` never imports `cells/`; `grid/` never imports any feature.
