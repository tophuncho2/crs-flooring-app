...
1. **Every primitive lives under exactly one bucket.** Buckets: `grid/`, `layout-grid/`, `fields/`, `cells/`, `dialogs/`, `nav/`, `badges/`, `headers/`, `features/`. Composites import down the dependency tree, never sideways: `cells/` may import `badges/` and `SelectDropdown` from the dropdowns engine (`@/engines/dropdowns`); `fields/` may import `layout-grid/` and `cells/`; `dialogs/` imports nothing from other buckets; `nav/` imports nothing from other buckets; `grid/` never imports any feature. (Dropdowns are no longer a bucket here — they moved to `@/engines/dropdowns`.)
...
- **dropdowns** — moved out to the dropdowns engine (`@/engines/dropdowns`): `SelectDropdown`, `AsyncRichDropdown`, `SegmentedDropdown`, `AnchoredPanel` + `useAsyncRichDropdownController`. `cells/dropdown-cell.tsx` composes `SelectDropdown` from there.
