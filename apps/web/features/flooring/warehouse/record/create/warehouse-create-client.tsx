...
function WarehouseCreatePanel({
  ...
  const controller = useSingleSectionCreateController<WarehouseDraft>({
    ...
    createRecord: async (localValue) => {
      ...
      const payload = await requestJson<{ warehouse: { id: string } }>("/api/flooring/warehouses", {
        method: "POST",
      ...
      return {
        redirectTo: buildRecordDetailHref("/dashboard/flooring/warehouse", payload.warehouse.id, backHref),
      }
    },
  })
