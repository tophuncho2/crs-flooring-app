    ...
    onSave: async (localValue, currentRecord) => {
      ...
      for (const row of normalizedSections) {
        if (isLocalOnlyRecordRow(row.id)) {
          const payload = await requestJson<{ section: { id: string; name: string; locationsCount: number } }>("/api/flooring/sections", {
            method: "POST",
        ...
        const payload = await requestJson<{ section: { id: string; name: string; locationsCount: number } }>(`/api/flooring/sections/${row.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            warehouseId: currentRecord.id,
      ...
      for (const serverRow of serverLocations) {
        ...
        await requestJson<{ ok: true }>(`/api/flooring/locations/${serverRow.id}`, {
          method: "DELETE",
        })
      }
      ...
      for (const row of normalizedResolvedLocations) {
        if (isLocalOnlyRecordRow(row.id)) {
          const payload = await requestJson<{ location: { id: string; locationCode: string; sectionId: string; sectionName: string | null } }>("/api/flooring/locations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              warehouseId: currentRecord.id,
        ...
        const payload = await requestJson<{ location: { id: string; locationCode: string; sectionId: string; sectionName: string | null } }>(`/api/flooring/locations/${row.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            locationCode: row.locationCode,
      ...
      for (const serverRow of serverSections) {
        ...
        await requestJson<{ ok: true }>(`/api/flooring/sections/${serverRow.id}`, {
          method: "DELETE",
        })
      }
