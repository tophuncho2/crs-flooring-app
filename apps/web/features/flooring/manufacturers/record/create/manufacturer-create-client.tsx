...
function ManufacturerCreatePanel({
  ...
  const controller = useSingleSectionCreateController<ManufacturerForm>({
    ...
    createRecord: async (localValue) => {
      const payload = await requestJson<{ manufacturer: ManufacturerRow }>("/api/flooring/manufacturers", {
        method: "POST",
      ...
      return {
        redirectTo: buildRecordDetailHref("/dashboard/flooring/manufacturers", payload.manufacturer.id, backHref),
      }
    },
  })
