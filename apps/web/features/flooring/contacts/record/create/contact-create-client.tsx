...
function ContactCreatePanel({
  ...
  const controller = useSingleSectionCreateController<ContactForm>({
    ...
    createRecord: async (localValue) => {
      const payload = await requestJson<{ contact: ContactDetail }>("/api/flooring/contacts", {
        method: "POST",
      ...
      return {
        redirectTo: buildRecordDetailHref("/dashboard/flooring/contacts", payload.contact.id, backHref),
      }
    },
  })
