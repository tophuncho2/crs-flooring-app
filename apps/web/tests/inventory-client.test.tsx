...
describe("InventoryClient", () => {
  ...
  it("uses the record-view engine with a primary section and a read-only cut-logs section", () => {
    ...
    expect(screen.getByText("Warehouse & Section")).toBeTruthy()
  ...
  it("saves inventory from the primary section while the footer remains delete/close only", async () => {
    ...
    const warehouseSectionField = screen.getByText("Warehouse & Section").closest("label")
    expect(warehouseSectionField).toBeTruthy()
    expect(warehouseSectionField?.textContent).toContain("Main Warehouse")
    expect(warehouseSectionField?.textContent).toContain("Reserve")
