...
import WorkOrdersClient from "@/modules/work-orders/components/work-orders-client"
...
vi.mock("@/modules/work-orders/record/panel/work-order-record-panel", workOrderRecordPanelMock)
...
vi.mock("@/modules/shared/ui/display/record-line-summary", () => ({
  RecordLineSummary: () => null,
}))

vi.mock("@/modules/shared/engines/record-view/shell/record-options-menu", () => ({
  RecordOptionsMenu: ({
    items,
  }: {
...
describe("WorkOrdersClient", () => {
  ...
  it("dashboard add routes to the canonical work order create form", async () => {
    ...
    expect(navigationMocks.push).toHaveBeenCalledWith(
      "/dashboard/work-orders/new?returnTo=%2Fdashboard%2Ftest",
      { scroll: false },
    )
  })
  ...
  it("creates a work order from the table-level sync flow and routes to the created detail page", async () => {
    ...
    await waitFor(() => {
      expect(requestJsonMock).toHaveBeenCalledWith("/api/work-orders", expect.objectContaining({
        method: "POST",
      }))
    })
    ...
    await waitFor(() => {
      expect(navigationMocks.push).toHaveBeenCalledWith(
        "/dashboard/work-orders/wo-2?returnTo=%2Fdashboard%2Ftest",
        { scroll: false },
      )
    })
  })
  ...
  it("shows completion success inside the canonical detail notice area", async () => {
    ...
    await waitFor(() => {
      expect(requestJsonMock).toHaveBeenCalledWith("/api/work-orders/wo-1", expect.objectContaining({
        method: "PATCH",
      }))
    })
  ...
  it("closes the detail page immediately when there are no unsaved changes", async () => {
    ...
    expect(navigationMocks.push).toHaveBeenCalledWith("/dashboard/work-orders", { scroll: false })
  })
