...
import { GET as GET_ALLOCATIONS, POST as POST_ALLOCATIONS } from "@/app/api/work-orders/[id]/items/[itemId]/allocations/route"
import { DELETE as DELETE_ALLOCATION, PATCH as PATCH_ALLOCATION } from "@/app/api/work-orders/[id]/items/[itemId]/allocations/[allocationId]/route"
import { GET as GET_ALLOCATION_OPTIONS } from "@/app/api/work-orders/[id]/items/[itemId]/allocation-options/route"
import { GET as GET_AUTO_ALLOCATION, POST as POST_AUTO_ALLOCATION } from "@/app/api/work-orders/[id]/auto-allocation/route"
...
vi.mock("@/modules/work-orders/application/allocations", () => ({
...
describe("work-order allocation routes", () => {
  ...
  it("lists and creates allocations", async () => {
    ...
    const listResponse = await GET_ALLOCATIONS(
      new Request(`http://localhost/api/work-orders/${WORK_ORDER_ID}/items/${ITEM_ID}/allocations`),
  ...
  it("updates and deletes allocations", async () => {
    ...
    const patchResponse = await PATCH_ALLOCATION(
      new Request(`http://localhost/api/work-orders/${WORK_ORDER_ID}/items/${ITEM_ID}/allocations/${ALLOCATION_ID}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          quantity: "5",
  ...
  it("lists allocation options and auto-allocation status", async () => {
    ...
    const optionsResponse = await GET_ALLOCATION_OPTIONS(
      new Request(`http://localhost/api/work-orders/${WORK_ORDER_ID}/items/${ITEM_ID}/allocation-options`),
      { params: Promise.resolve({ id: WORK_ORDER_ID, itemId: ITEM_ID }) },
    )
    expect((await optionsResponse.json()).options).toHaveLength(1)

    const statusResponse = await GET_AUTO_ALLOCATION(
      new Request(`http://localhost/api/work-orders/${WORK_ORDER_ID}/auto-allocation`),
