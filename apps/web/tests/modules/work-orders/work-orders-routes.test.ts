...
import { GET as GET_ITEMS, POST as POST_ITEM } from "@/app/api/work-orders/[id]/items/route"
import { DELETE as DELETE_ITEM, PATCH as PATCH_ITEM } from "@/app/api/work-orders/[id]/items/[itemId]/route"
import { GET as GET_SERVICE_ITEMS, POST as POST_SERVICE_ITEM } from "@/app/api/work-orders/[id]/service-items/route"
import { DELETE as DELETE_SERVICE_ITEM, PATCH as PATCH_SERVICE_ITEM } from "@/app/api/work-orders/[id]/service-items/[itemId]/route"
import { GET as GET_SALES_REPS, POST as POST_SALES_REP } from "@/app/api/work-orders/[id]/sales-reps/route"
import { GET as GET_CALCULATIONS } from "@/app/api/work-orders/[id]/calculations/route"
import { DELETE as DELETE_SALES_REP, PATCH as PATCH_SALES_REP } from "@/app/api/work-orders/[id]/sales-reps/[repId]/route"
...
vi.mock("@/modules/work-orders/transport/detail", () => ({
...
vi.mock("@/modules/work-orders/mutations", () => ({
...
describe("work-order child routes", () => {
  ...
  it("child item routes list, create, patch, and delete material items", async () => {
    ...
    const listResponse = await GET_ITEMS(new Request("http://localhost/api/work-orders/wo-1/items"), {
      params: Promise.resolve({ id: "wo-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_ITEM(
      new Request("http://localhost/api/work-orders/wo-1/items", {
        method: "POST",
    ...
    const patchResponse = await PATCH_ITEM(
      new Request("http://localhost/api/work-orders/wo-1/items/item-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          quantity: "3",
    ...
    const deleteResponse = await DELETE_ITEM(new Request("http://localhost/api/work-orders/wo-1/items/item-1", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mutation: {
  ...
  it("child service-item routes list, create, patch, and delete service items", async () => {
    ...
    const listResponse = await GET_SERVICE_ITEMS(new Request("http://localhost/api/work-orders/wo-1/service-items"), {
      params: Promise.resolve({ id: "wo-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_SERVICE_ITEM(
      new Request("http://localhost/api/work-orders/wo-1/service-items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          serviceId: "svc-1",
    ...
    const patchResponse = await PATCH_SERVICE_ITEM(
      new Request("http://localhost/api/work-orders/wo-1/service-items/svc-item-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          quantity: "2",
    ...
    const deleteResponse = await DELETE_SERVICE_ITEM(new Request("http://localhost/api/work-orders/wo-1/service-items/svc-item-1", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mutation: {
  ...
  it("child sales-rep routes list, create, patch, and delete rep rows", async () => {
    ...
    const listResponse = await GET_SALES_REPS(new Request("http://localhost/api/work-orders/wo-1/sales-reps"), {
      params: Promise.resolve({ id: "wo-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_SALES_REP(
      new Request("http://localhost/api/work-orders/wo-1/sales-reps", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const patchResponse = await PATCH_SALES_REP(
      new Request("http://localhost/api/work-orders/wo-1/sales-reps/rep-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          percent: "15",
    ...
    const deleteResponse = await DELETE_SALES_REP(new Request("http://localhost/api/work-orders/wo-1/sales-reps/rep-1", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        mutation: {
  ...
  it("child calculation route lists derived rows", async () => {
    ...
    const response = await GET_CALCULATIONS(new Request("http://localhost/api/work-orders/wo-1/calculations"), {
      params: Promise.resolve({ id: "wo-1" }),
    })
    const payload = await response.json()
