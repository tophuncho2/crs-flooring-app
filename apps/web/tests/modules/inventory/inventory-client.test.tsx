...
describe("InventoryClient", () => {
  ...
  it("renders clickable rows, colored import pills, and routes to the canonical detail page", async () => {
    ...
    expect(navigationMocks.push).toHaveBeenCalledWith(
      "/dashboard/inventory/inv-1?returnTo=%2Fdashboard%2Ftest",
      { scroll: false },
    )
  })
  ...
  it("uses the record-view engine with a primary section and a read-only cut-logs section", () => {
    ...
    const warehouseField = screen.getByText("Warehouse").closest("label")
    expect(warehouseField).toBeTruthy()
    expect(warehouseField?.textContent).toContain("Main Warehouse")
  ...
  it("saves inventory from the primary section while the footer remains delete/close only", async () => {
    ...
    await waitFor(() => {
      expect(requestJsonMock).toHaveBeenCalledWith("/api/inventory/inv-1", expect.objectContaining({
        method: "PATCH",
      }))
    })
    ...
    expect(warehouseField).toBeTruthy()
    expect(warehouseField?.textContent).toContain("Main Warehouse")
    const sectionField = screen.getByText("Section").closest("label")
    expect(sectionField).toBeTruthy()
    expect(sectionField?.textContent).toContain("Reserve")
  ...
  it("updates the inventory URL filters without keeping the previous page param", async () => {
    const user = userEvent.setup()
    window.history.replaceState({}, "", "/dashboard/test?page=3")
  ...
  it("adds a draft cut-log row and saves it from the managed section", async () => {
    ...
    expect(requestJsonMock).toHaveBeenNthCalledWith(2, "/api/inventory/inv-cut")
