...
import { DELETE as DELETE_TEMPLATE, PATCH as PATCH_TEMPLATE } from "@/app/api/templates/[id]/route"
import { GET as GET_ITEMS, POST as POST_ITEM } from "@/app/api/templates/[id]/items/route"
import { DELETE as DELETE_ITEM, PATCH as PATCH_ITEM } from "@/app/api/templates/[id]/items/[itemId]/route"
import { GET as GET_SERVICE_ITEMS, POST as POST_SERVICE_ITEM } from "@/app/api/templates/[id]/service-items/route"
import { DELETE as DELETE_SERVICE_ITEM, PATCH as PATCH_SERVICE_ITEM } from "@/app/api/templates/[id]/service-items/[itemId]/route"
import { GET as GET_SALES_REPS, POST as POST_SALES_REP } from "@/app/api/templates/[id]/sales-reps/route"
import { GET as GET_CALCULATIONS } from "@/app/api/templates/[id]/calculations/route"
import { DELETE as DELETE_SALES_REP, PATCH as PATCH_SALES_REP } from "@/app/api/templates/[id]/sales-reps/[repId]/route"
...
vi.mock("@/modules/shared/access/templates-work-orders", () => ({
...
vi.mock("@/modules/templates/application/manage-template", () => ({
...
vi.mock("@/modules/templates/mutations", () => ({
...
describe("template routes", () => {
  ...
  it("GET /api/templates returns normalized template rows", async () => {
  ...
  it("PATCH updates allowed fields and returns normalized payload", async () => {
    ...
    const response = await PATCH_TEMPLATE(
      new Request(`http://localhost/api/templates/${templateId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          templateTag: "Make Ready",
  ...
  it("child item routes list, create, patch, and delete material items", async () => {
    ...
    const listResponse = await GET_ITEMS(new Request("http://localhost/api/templates/tpl-1/items"), {
      params: Promise.resolve({ id: "tpl-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_ITEM(
      new Request("http://localhost/api/templates/tpl-1/items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const patchResponse = await PATCH_ITEM(
      new Request("http://localhost/api/templates/tpl-1/items/item-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    const deleteResponse = await DELETE_ITEM(new Request("http://localhost/api/templates/tpl-1/items/item-1"), {
      params: Promise.resolve({ id: "tpl-1", itemId: "item-1" }),
    })
  ...
  it("child service-item routes list, create, patch, and delete service items", async () => {
    ...
    const listResponse = await GET_SERVICE_ITEMS(new Request("http://localhost/api/templates/tpl-1/service-items"), {
      params: Promise.resolve({ id: "tpl-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_SERVICE_ITEM(
      new Request("http://localhost/api/templates/tpl-1/service-items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const patchResponse = await PATCH_SERVICE_ITEM(
      new Request("http://localhost/api/templates/tpl-1/service-items/svc-item-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    const deleteResponse = await DELETE_SERVICE_ITEM(new Request("http://localhost/api/templates/tpl-1/service-items/svc-item-1"), {
      params: Promise.resolve({ id: "tpl-1", itemId: "svc-item-1" }),
    })
    expect((await deleteResponse.json()).ok).toBe(true)
  ...
  it("child sales-rep routes list, create, patch, and delete sales reps", async () => {
    ...
    const listResponse = await GET_SALES_REPS(new Request("http://localhost/api/templates/tpl-1/sales-reps"), {
      params: Promise.resolve({ id: "tpl-1" }),
    })
    expect((await listResponse.json()).items).toHaveLength(1)

    const createResponse = await POST_SALES_REP(
      new Request("http://localhost/api/templates/tpl-1/sales-reps", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const patchResponse = await PATCH_SALES_REP(
      new Request("http://localhost/api/templates/tpl-1/sales-reps/rep-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    const deleteResponse = await DELETE_SALES_REP(new Request("http://localhost/api/templates/tpl-1/sales-reps/rep-1"), {
      params: Promise.resolve({ id: "tpl-1", repId: "rep-1" }),
    })
    expect((await deleteResponse.json()).ok).toBe(true)
  ...
  it("child calculation route lists derived rows", async () => {
    ...
    const response = await GET_CALCULATIONS(new Request("http://localhost/api/templates/tpl-1/calculations"), {
      params: Promise.resolve({ id: "tpl-1" }),
    })
    const payload = await response.json()
