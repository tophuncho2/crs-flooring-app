...
describe("ManagementCompanyDetailClient", () => {
  ...
  it("renders through the record-view engine with read-only linked properties and nested templates", async () => {
    ...
    window.history.replaceState({}, "", "/dashboard/management-companies/mc-1")

    render(
      <ManagementCompanyDetailClient
        company={company}
        backHref="/dashboard/management-companies"
      />,
    )
    ...
    await waitFor(() => {
      expect(navigationMocks.push).toHaveBeenCalledWith(
        "/dashboard/properties/new?returnTo=%2Fdashboard%2Ftest&managementCompanyId=mc-1",
        { scroll: false },
      )
    })
    ...
    await waitFor(() => {
      expect(navigationMocks.push).toHaveBeenCalledWith(
        expect.stringContaining("/dashboard/properties/prop-1"),
        { scroll: false },
      )
    })
    ...
    await waitFor(() => {
      expect(navigationMocks.push).toHaveBeenCalledWith(
        expect.stringContaining("/dashboard/templates/tpl-1"),
        { scroll: false },
      )
    })
  })
