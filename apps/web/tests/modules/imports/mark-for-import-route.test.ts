import { beforeEach, describe, expect, it, vi } from "vitest"

// Golden-master for the non-standard mark-for-import mutation: status 202, and a
// response body that re-fetches the PARENT import AFTER the use case so the
// client can resync the aggregate root's OCC token. entityId is the parent id.

const {
  markStagedRowsForImportUseCaseMock,
  getImportDetailByIdMock,
  applyRoutePolicyMock,
  enforceMutationReceiptMock,
  finalizeMutationReceiptMock,
  withMutationTelemetryMock,
  telemetryCalls,
} = vi.hoisted(() => ({
  markStagedRowsForImportUseCaseMock: vi.fn(),
  getImportDetailByIdMock: vi.fn(),
  applyRoutePolicyMock: vi.fn(),
  enforceMutationReceiptMock: vi.fn(),
  finalizeMutationReceiptMock: vi.fn(),
  withMutationTelemetryMock: vi.fn(),
  telemetryCalls: [] as Array<Record<string, unknown>>,
}))

vi.mock("@builders/application", async () => {
  const actual = await vi.importActual<typeof import("@builders/application")>("@builders/application")
  return { ...actual, markStagedRowsForImportUseCase: markStagedRowsForImportUseCaseMock }
})

vi.mock("@builders/db", async () => {
  const actual = await vi.importActual<typeof import("@builders/db")>("@builders/db")
  return { ...actual, getImportDetailById: getImportDetailByIdMock }
})

vi.mock("@/server/http/route-policy", async () => {
  const actual = await vi.importActual<typeof import("@/server/http/route-policy")>("@/server/http/route-policy")
  return {
    ...actual,
    applyRoutePolicy: applyRoutePolicyMock,
    enforceMutationReceipt: enforceMutationReceiptMock,
    finalizeMutationReceipt: finalizeMutationReceiptMock,
  }
})

vi.mock("@/server/telemetry/mutation-telemetry", () => ({
  withMutationTelemetry: withMutationTelemetryMock,
}))

vi.mock("@/server/platform/logger", () => ({ logEvent: vi.fn() }))

const { POST } = await import(
  "@/app/api/imports/[id]/staged-inventory-rows/mark-for-import/route"
)

const IMPORT_ID = "import-42"

beforeEach(() => {
  vi.clearAllMocks()
  telemetryCalls.length = 0
  applyRoutePolicyMock.mockResolvedValue({
    requestId: "req-1",
    clientIp: "127.0.0.1",
    user: { id: "admin-1", email: "admin@test.com", rank: "DEVELOPER" },
  })
  enforceMutationReceiptMock.mockResolvedValue({ replay: null, requestHash: "hash-1" })
  finalizeMutationReceiptMock.mockResolvedValue(undefined)
  withMutationTelemetryMock.mockImplementation(
    async (_ctx: unknown, options: Record<string, unknown>, operation: () => Promise<unknown>) => {
      telemetryCalls.push(options)
      return operation()
    },
  )
})

describe("POST /api/imports/[id]/staged-inventory-rows/mark-for-import", () => {
  it("returns 202 { batch, import } with the re-fetched parent and parent entityId", async () => {
    markStagedRowsForImportUseCaseMock.mockResolvedValue({ id: "batch-1" })
    getImportDetailByIdMock.mockResolvedValue({ id: IMPORT_ID, updatedAt: "2026-03-19T00:00:00.000Z" })

    const response = await POST(
      new Request(
        `http://localhost/api/imports/${IMPORT_ID}/staged-inventory-rows/mark-for-import`,
        {
          method: "POST",
          body: JSON.stringify({ stagedRowIds: ["r1", "r2"], mutation: { idempotencyKey: "idem-1" } }),
        },
      ),
      { params: Promise.resolve({ id: IMPORT_ID }) },
    )
    const payload = await response.json()

    expect(response.status).toBe(202)
    expect(payload).toEqual({
      batch: { id: "batch-1" },
      import: { id: IMPORT_ID, updatedAt: "2026-03-19T00:00:00.000Z" },
    })
    expect(markStagedRowsForImportUseCaseMock).toHaveBeenCalledWith(IMPORT_ID, ["r1", "r2"], {
      userId: "admin-1",
      userEmail: "admin@test.com",
    })
    expect(getImportDetailByIdMock).toHaveBeenCalledWith(IMPORT_ID)
    expect(finalizeMutationReceiptMock).toHaveBeenCalledWith(
      expect.objectContaining({
        scope: "imports.staged-inventory-rows.mark-for-import",
        responseStatus: 202,
      }),
    )
    expect(telemetryCalls[0]).toMatchObject({
      action: "imports.staged-inventory-rows.mark-for-import",
      entityType: "flooringImportEntry",
      entityId: IMPORT_ID,
    })
  })
})
