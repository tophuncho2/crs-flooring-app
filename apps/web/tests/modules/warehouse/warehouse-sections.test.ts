...
import { DELETE, PATCH } from "@/app/api/sections/[id]/route"
...
describe("warehouse sections routes", () => {
  ...
  it("GET returns normalized rows with locationsCount", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/sections"))
  ...
  it("GET filters correctly by warehouseId", async () => {
    ...
    await GET(new Request("http://localhost/api/sections?warehouseId=wh-9"))
  ...
  it("PATCH updates name and returns normalized payload", async () => {
    ...
    const response = await PATCH(
      new Request("http://localhost/api/sections/sec-1", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  it("DELETE returns 404 when section is missing", async () => {
    ...
    const response = await DELETE(new Request("http://localhost/api/sections/sec-1"), {
      params: Promise.resolve({ id: "sec-1" }),
    })
    const payload = await response.json()
