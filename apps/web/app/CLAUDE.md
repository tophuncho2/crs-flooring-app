...
`apps/web/app/api/{module}/[id]/{scope}/route.ts`                           — e.g. GET /indicators (product low-stock indicators)


Used for read-only helpers scoped to a specific record or row (signed download URLs, scoped record reads, etc.).
