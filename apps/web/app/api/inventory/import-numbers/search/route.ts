...
export async function GET(request: Request) {
  ...
  const rateLimited = await enforceQueryRateLimit(
    request,
    access,
    "/api/inventory/import-numbers/search",
  )
  if (rateLimited) return rateLimited

  try {
    const url = new URL(request.url)
    const input = validateInventoryImportNumberSearchQuery(url.searchParams)
    const result = await searchInventoryImportNumbersUseCase(input)
