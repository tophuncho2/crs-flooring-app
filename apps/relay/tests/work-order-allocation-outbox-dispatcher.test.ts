...
describe("createWorkOrderAllocationOutboxDispatcher", () => {
  ...
  it("treats a duplicate BullMQ job id as an idempotent dispatch", async () => {
    const markEventDispatched = vi.fn().mockResolvedValue(undefined)
    const queueRun = vi.fn().mockResolvedValue(true)
    const dispatcher = createWorkOrderAllocationOutboxDispatcher({
      listClaimableEvents: vi.fn().mockResolvedValue([
        {
          id: "event-1",
    ...
    const queue = {
      ...
      getJob: vi.fn().mockResolvedValue({ id: toBullMqJobId("work-order-allocation:v1:11111111-1111-4111-8111-111111111111") }),
    }
    ...
    expect(markEventDispatched).toHaveBeenCalled()
  })
})
