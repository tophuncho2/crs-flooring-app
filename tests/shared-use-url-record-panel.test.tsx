...
const { navigationState } = vi.hoisted(() => ({
  navigationState: {
    pathname: "/dashboard/flooring/services",
    pushMock: vi.fn(),
...
describe("useUrlRecordPanel", () => {
  beforeEach(() => {
    navigationState.pathname = "/dashboard/flooring/services"
  ...
  it("pushes the record param while preserving unrelated query params", () => {
    ...
    expect(navigationState.pushMock).toHaveBeenCalledWith(
      "/dashboard/flooring/services?tab=active&page=2&service=svc-3",
      { scroll: false },
    )
  })
  ...
  it("removes only the target query param when the record is closed", () => {
    ...
    expect(navigationState.pushMock).toHaveBeenCalledWith(
      "/dashboard/flooring/services?tab=active&page=2",
      { scroll: false },
    )
  })
})
